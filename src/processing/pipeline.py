...
Runs the full Phase 2 pipeline in sequence:
  ...
  4. Feature engineering   → complaint_daily_agg
                           → kpi_daily_agg
                           → feature_matrix (joined)
  5. Train/test split      → X_train, X_test, y_train, y_test
...
# FIX PL2: corrected module name (was real_data_loader)
# FIX PL1: explicitly import the two functions that were missing
from src.ingestion.data_loader import load_complaints, load_kpi_data

# FIX PL4: renamed module (was real_complaint_cleaner)
from src.processing.complaint_cleaner import (
    clean_complaints,
    print_cleaning_report as print_complaint_report,
)

# FIX PL3: lowercase module name (was Kpi_cleaner — fails on Linux/Mac)
from src.processing.kpi_cleaner import (
    clean_kpi_data,
    print_cleaning_report as print_kpi_report,
)
...
def run_pipeline(
    ...
    verbose          : whether to print cleaning reports
    ...
    complaints_raw = load_complaints()   # FIX PL1: now correctly imported
    kpi_raw        = load_kpi_data()     # FIX PL1
    ...
    kpi_clean, kpi_report = clean_kpi_data(
        kpi_raw, outlier_strategy=outlier_strategy
    )
    if verbose:
    ...
    logger.info("=" * 60)
    logger.info(
        f"  Cleaned complaints  : {len(complaints_clean):>8,} rows × "
        f"{complaints_clean.shape[1]} cols"
    )
    logger.info(
        f"  Cleaned KPI data    : {len(kpi_clean):>8,} rows × "
        f"{kpi_clean.shape[1]} cols"
    )
    logger.info(
        f"  Complaint daily agg : {len(complaint_agg):>8,} rows × "
        f"{complaint_agg.shape[1]} cols"
    )
    logger.info(
        f"  KPI daily agg       : {len(kpi_agg):>8,} rows × "
        f"{kpi_agg.shape[1]} cols"
    )
    logger.info(
        f"  Feature matrix      : {len(feature_matrix):>8,} rows × "
