...
// ─────────────────────────────────────────────────────────────────────
// Responsive customer complaint / feedback portal
//
// Changes from original:
//   - Fully responsive: mobile-first CSS, 3 breakpoints
//   - Emoji icons in ClassificationResult → Lucide React
//   - Touch-friendly inputs (min 44px tap targets)
//   - Inline field validation with per-field error hints
//   - MSISDN format validation (Tunisian 8-digit format)
//   - Character count progress bar in textarea
//   - Section grouping (Contact | Account | Message)
//   - Smooth result card entrance animation
//   - All FIX-1 / FIX-2 / FIX-3 logic preserved exactly
// ─────────────────────────────────────────────────────────────────────
...
const L = {
  fr: {
    ...
    msisdnHint:  'Format: 216XXXXXXXX',
    msisdnError: 'Numéro invalide. Format attendu: 216XXXXXXXX',
    city:        'Ville',
    cityPh:      'Tunis, Sfax, Sousse…',
    segment:     'Segment',
    segmentPh:   '-- Sélectionner --',
    channel:     'Canal',
    channelWeb:  'Portail Web',
    channelApp:  'App Mobile',
    channelSocial:'Réseaux Sociaux',
    textLabel:   'Décrivez votre problème ou demande',
    placeholder: 'Ex: Mon réseau 4G coupe à Sfax depuis 3 jours et je ne peux plus passer d\'appels…\n\nOu: Comment activer le roaming international ?',
    hint:        'Langue et type détectés automatiquement',
    charLimit:   '/ 3000',
    submit:      'Envoyer',
    ...
    reclamationSub:  'Problème de service détecté automatiquement',
    reclamationNext: 'Notre équipe traitera votre réclamation dans les meilleurs délais.',
    feedback:        'Feedback',
    feedbackSub:     'Message de retour ou demande d\'information',
    feedbackNext:    'Merci pour votre message. Il a bien été enregistré.',
    pending:         'Classification en attente',
    pendingSub:      'Le type n\'a pas encore été déterminé.',
    ref:    'Référence',
    lang:   'Langue',
    cat:    'Catégorie',
    sent:   'Sentiment',
    urg:    'Urgence',
    city2:  'Ville détectée',
    resp:   'Délai de réponse',
  },
  ar: {
    title:     'بوابة العملاء',
    ...
    msisdn:      'رقم الهاتف',
    msisdnHint:  'الصيغة: 216XXXXXXXX',
    msisdnError: 'رقم غير صحيح. الصيغة المطلوبة: 216XXXXXXXX',
    city:        'المدينة',
    cityPh:      'تونس، صفاقس، سوسة…',
    segment:     'الشريحة',
    segmentPh:   '-- اختر --',
    channel:     'القناة',
    channelWeb:  'البوابة الإلكترونية',
    channelApp:  'تطبيق الجوال',
    channelSocial:'وسائل التواصل',
    textLabel:   'صف مشكلتك أو طلبك',
    placeholder: 'مثال: شبكتي مقطوعة في تونس منذ 3 أيام ولا أستطيع إجراء مكالمات…\n\nأو: كيف يمكنني تفعيل التجوال الدولي؟',
    hint:        'تُكشف اللغة والنوع تلقائياً',
    charLimit:   '/ 3000',
    submit:      'إرسال',
    ...
    reclamation:     'شكوى',
    reclamationSub:  'تم رصد مشكلة في الخدمة تلقائياً',
    reclamationNext: 'سيتولى فريقنا معالجة شكواك في أقرب وقت ممكن.',
    feedback:        'تعليق',
    feedbackSub:     'رسالة إيجابية أو طلب معلومات',
    feedbackNext:    'شكراً على رسالتك. تم تسجيلها بنجاح.',
    pending:         'التصنيف قيد الانتظار',
    pendingSub:      'لم يتم تحديد النوع بعد.',
    ref: 'المرجع', lang: 'اللغة', cat: 'الفئة',
    sent: 'المشاعر', urg: 'الأولوية', city2: 'المدينة المكتشفة', resp: 'وقت الاستجابة',
  },
  en: {
    ...
    msisdnHint:  'Format: 216XXXXXXXX',
    msisdnError: 'Invalid number. Expected format: 216XXXXXXXX',
    city:        'City',
    cityPh:      'Tunis, Sfax, Sousse…',
    segment:     'Segment',
    segmentPh:   '-- Select --',
    ...
    channelSocial:'Social Networks',
    textLabel:   'Describe your issue or request',
    placeholder: 'Ex: My 4G network keeps dropping in Tunis since yesterday and I cannot make calls…\n\nOr: How do I activate international roaming?',
    hint:        'Language and type auto-detected',
    charLimit:   '/ 3000',
    submit:      'Send',
    ...
    reclamationSub:  'Service problem automatically detected',
    reclamationNext: 'Our team will handle your complaint as soon as possible.',
    feedback:        'Feedback',
    feedbackSub:     'Positive comment or information request',
    feedbackNext:    'Thank you for your message. It has been recorded.',
    pending:         'Classification pending',
    pendingSub:      'Type has not been determined yet.',
    ref: 'Reference', lang: 'Language', cat: 'Category',
    sent: 'Sentiment', urg: 'Urgency', city2: 'Detected city', resp: 'Response time',
  },
}
...
function validateMsisdn(v) {
  ...
  return /^216[0-9]{8}$/.test(v.replace(/\s/g, ''))
}

// ═══════════════════════════════════════════════════════════════════════
// CLASSIFICATION RESULT CARD
// FIX-1 / FIX-2 / FIX-3 logic preserved exactly
// ═══════════════════════════════════════════════════════════════════════
function ClassificationResult({ result, labels }) {
  const rawValue = result?.is_complaint
  const state    = rawValue === true  ? 'reclamation'
                 : rawValue === false ? 'feedback'
                 : 'pending'
  ...
  const TOKEN = {
    reclamation: {
      Icon:        AlertTriangle,
      iconColor:   C.red,
      headline:    labels.reclamation,
      sub:         labels.reclamationSub,
      next:        labels.reclamationNext,
      NextIcon:    ClipboardList,
      accent:      C.red,
      accentLight: C.redLight,
      accentBg:    'rgba(239,68,68,.06)',
      accentBorder:'rgba(239,68,68,.25)',
    },
    feedback: {
      Icon:        Info,
      iconColor:   C.blue,
      headline:    labels.feedback,
      sub:         labels.feedbackSub,
      next:        labels.feedbackNext,
      NextIcon:    CheckCircle2,
      accent:      C.blue,
      accentLight: C.blueLight,
      accentBg:    'rgba(59,130,246,.06)',
      accentBorder:'rgba(59,130,246,.25)',
    },
    pending: {
      Icon:        Clock,
      iconColor:   C.textMuted,
      headline:    labels.pending,
      sub:         labels.pendingSub,
      next:        '',
      NextIcon:    null,
      accent:      C.textMuted,
      accentLight: C.textMuted,
      accentBg:    'rgba(255,255,255,.03)',
  ...
  return (
    <div className="cf-result-card" style={{
      marginTop: 20,
      border: `1px solid ${tk.accentBorder}`,
      borderRadius: 12,
      overflow: 'hidden',
      background: tk.accentBg,
      animation: 'cf-slideIn .35s cubic-bezier(.22,1,.36,1)',
    }}>
      {/* Banner */}
      <div className="cf-result-banner" style={{
        padding: '18px 20px',
        borderBottom: `1px solid ${tk.accentBorder}`,
        display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap',
      }}>
        <div style={{
          width: 48, height: 48, flexShrink: 0,
          background: `${tk.accent}15`,
          border: `2px solid ${tk.accent}35`,
          borderRadius: 10,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            fontSize: 20, fontWeight: 900, color: tk.accentLight,
            letterSpacing: '-.3px', lineHeight: 1.1, marginBottom: 4,
        ...
        <div className="cf-result-ref" style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 9, color: C.textMuted, letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: 3 }}>
            {labels.ref}
          </div>
          <div style={{
            fontFamily: "'Barlow Condensed', monospace",
            fontSize: 15, fontWeight: 800, color: tk.accentLight, letterSpacing: '-.2px',
          }}>
            {result.complaint_id}
          </div>
        </div>
      ...
      {/* Detail grid */}
      <div className="cf-detail-grid" style={{
        padding: '14px 20px',
        display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px 20px',
      }}>
        <ResultDetail
          label={labels.lang}
          value={result.language_detected}
          Icon={Globe}
          color={result.language_detected === 'العربية' ? C.red : result.language_detected === 'Français' ? C.cyan : C.green}
        />
        ...
        <ResultDetail
          ...
          Icon={Zap}
          color={result.sentiment === 'critique' ? C.red : result.sentiment === 'négatif' ? C.amber : result.sentiment === 'positif' ? C.green : C.textMuted}
        />
        {state === 'reclamation' && result.urgency_level && (
          <div>
            <div style={detailLabelStyle}>
              <Clock size={9} style={{ display: 'inline', marginRight: 4 }}/>{labels.urg}
            </div>
            <span style={{
              display: 'inline-block',
              ...(URGENCY_STYLE[result.urgency_level] || URGENCY_STYLE['normal']),
        ...
        {result.city_detected && (
          <ResultDetail label={labels.city2} value={result.city_detected} Icon={MapPin} color={C.green}/>
        )}
        {state === 'reclamation' && result.estimated_response_hours != null && (
          <ResultDetail label={labels.resp} value={`${result.estimated_response_hours}h`} Icon={Clock} color={tk.accentLight}/>
        )}
      </div>
...
function ResultDetail({ label, value, Icon, color }) {
  return (
    <div>
      <div style={detailLabelStyle}>
        {Icon && <Icon size={9} style={{ display: 'inline', marginRight: 4 }}/>}
        {label}
      </div>
...
function SectionDivider({ label }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '22px 0 16px' }}>
      <span style={{ fontSize: 9, fontWeight: 800, color: C.textDim, letterSpacing: '2px', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
        {label}
      </span>
...
// ── Language flag codes ────────────────────────────────────────────────
// Use CSS-flag emoji in span to prevent sizing issues
const LANG_TABS = [
...
export default function ComplaintForm() {
  ...
  // Character count — colour thresholds
    ...
    <>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* 2-column grid — collapsed on mobile */
        .cf-grid-2 {
          display: grid;
        ...
        @media (min-width: 480px) {
          ...
          .cf-card { min-height: auto; border-radius: 16px; box-shadow: 0 12px 48px rgba(0,0,0,.5); }
        }
        ...
        @media (min-width: 768px) {
          ...
          .cf-lang-btn { font-size: 14px; }
        }

        /* ── SMALL MOBILE (< 360px) ─────────────────────────── */
        @media (max-width: 359px) {
          .cf-grid-2  { grid-template-columns: 1fr; }
          .cf-card    { padding: 24px 18px 32px; }
          .cf-lang-btn { font-size: 12px; padding: 8px 10px; }
        }
      `}</style>
        ...
        <div className="cf-card">
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <div style={{
              ...
              marginBottom: 14, boxShadow: `0 6px 20px ${C.primary}40`,
            }}>
              O
            </div>
            <h1 style={{ color: C.text, fontSize: 22, fontWeight: 700, margin: '0 0 5px', letterSpacing: '-.3px' }}>
              {labels.title}
            </h1>
          ...
          <div className="cf-lang-tabs" role="tablist">
            {LANG_TABS.map(({ code, flag, label }) => (
              <button
                key={code}
                className={`cf-lang-btn${lang === code ? ' active' : ''}`}
          ...
          <form onSubmit={handleSubmit} noValidate>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    if (fieldErrors.msisdn) setFieldErrors(fe => ({ ...fe, msisdn: '' }))
                  }}
                  placeholder={labels.msisdnHint}
                  {...inp}
                  style={{ ...inp.style, borderColor: fieldErrors.msisdn ? C.red : C.border }}
                />
                {fieldErrors.msisdn && (
                  <div className="cf-field-error">
                    <AlertTriangle size={10}/> {fieldErrors.msisdn}
                  </div>
                )}
            ...
            <div style={{ marginBottom: 6 }}>
              ...
              <textarea
                ...
                onChange={e => {
                  ...
                  if (fieldErrors.text) setFieldErrors(fe => ({ ...fe, text: '' }))
                }}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
                ...
                <span style={{ fontSize: 10, color: charColor, fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>
                  {charCount} {labels.charLimit}
                </span>
              </div>

              {/* Progress bar */}
              <div className="cf-char-bar">
                <div className="cf-char-bar-fill" style={{
                  width: `${charPct}%`,
                  background: charCount > 2800 ? C.amber : charCount > 50 ? C.primary : 'rgba(255,255,255,.1)',
                }}/>
              </div>

              {fieldErrors.text && (
                <div className="cf-field-error" style={{ marginTop: 6 }}>
                  <AlertTriangle size={10}/> {fieldErrors.text}
                </div>
              )}
            </div>
            ...
            {error && (
              <div style={{
                ...
                color: '#FCA5A5', fontSize: 12, marginBottom: 4,
