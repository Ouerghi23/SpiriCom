...
//   - Client-side SEARCH BAR (ID, text, category, city, MSISDN)
//   - AUTO-REFRESH every 30 s (timer visible in Refresh button)
//   - VISIBLE ERROR BANNER when fetch fails (was silently swallowing)
//   - Last-refreshed timestamp next to Refresh button
//   - "X résultats / Y total" shown while searching
//   - refreshTrigger state drives both auto and manual refresh
// ─────────────────────────────────────────────────────────────────────
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:C.bg2,
        ...
        <div style={{ padding:'20px 24px', display:'flex', flexDirection:'column', gap:18 }}>
          ...
          <div style={{ background:C.bg3, border:`1px solid ${C.border}`, padding:'14px 16px' }}>
            <div style={{ fontSize:9, color:C.textDim, letterSpacing:'2px', fontWeight:800,
              textTransform:'uppercase', marginBottom:10 }}>{t('nlp.popupText')}</div>
            <p style={{ fontSize:13, color:C.text, lineHeight:1.7, margin:0, wordBreak:'break-word' }}>
              {c.text_original}
            </p>
          </div>
          ...
          <div>
            <div style={{ fontSize:9, color:C.textDim, letterSpacing:'2px', fontWeight:800,
              textTransform:'uppercase', marginBottom:10 }}>{t('nlp.popupNlp')}</div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:1,
              ...
              {[
                ...
                <div key={label} style={{ background:C.bg3, padding:'12px 14px',
                  position:'relative', overflow:'hidden' }}>
                  <div style={{ position:'absolute', top:0, left:'10%', right:'10%', height:1,
                    background:`linear-gradient(90deg,transparent,${color},transparent)` }}/>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:6 }}>
                    <Icon size={10} color={color}/>
                    <span style={{ fontSize:9, color:C.textDim, letterSpacing:'1.5px',
                      fontWeight:700, textTransform:'uppercase' }}>{label}</span>
                  </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <span style={{ fontSize:9, color:C.textDim, letterSpacing:'2px',
              fontWeight:800, textTransform:'uppercase' }}>{t('nlp.tableUrg')}</span>
            <span style={{ ...urgStyle, padding:'4px 12px', fontSize:9,
          ...
          {keywords.length > 0 && (
            <div>
              <div style={{ fontSize:9, color:C.textDim, letterSpacing:'2px',
                fontWeight:800, textTransform:'uppercase', marginBottom:8, display:'flex', alignItems:'center', gap:6 }}>
                <Key size={10} color={C.textDim}/>{t('nlp.popupKeywords')}
              </div>
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {keywords.map((kw,i)=>(
                  <span key={i} style={{ fontSize:10, padding:'3px 10px', fontWeight:600,
                    background:'rgba(255,255,255,.05)', border:`1px solid ${C.border}`, color:C.textMuted }}>
                    {kw}
                  </span>
                ))}
              </div>
          ...
          <div>
            <div style={{ fontSize:9, color:C.textDim, letterSpacing:'2px',
              fontWeight:800, textTransform:'uppercase', marginBottom:10 }}>{t('nlp.popupMeta')}</div>
          ...
          <div style={{ display:'flex', gap:8, paddingTop:8,
            ...
            {c.status!=='in_progress'&&c.status!=='resolved'&&(
              <ActionBtn onClick={()=>onStatusUpdate(c.complaint_id,'in_progress')}
                disabled={isActioning} variant="amber" small={false}>{t('nlp.enCours')}</ActionBtn>
            )}
            {c.status!=='resolved'&&(
              <ActionBtn onClick={()=>onStatusUpdate(c.complaint_id,'resolved')}
                disabled={isActioning} variant="green" small={false}>
                <Check size={12}/> {t('nlp.cloture')}
              </ActionBtn>
            )}
            {confirmDelete===c.complaint_id?(
              <>
                <ActionBtn onClick={()=>onDelete(c.complaint_id)} disabled={isActioning}
                  variant="redSolid" small={false}>{t('nlp.confirmer')}</ActionBtn>
                <ActionBtn onClick={()=>setConfirmDelete(null)} variant="ghost" small={false}>
                  <X size={12}/>
                </ActionBtn>
              </>
            ):(
              <ActionBtn onClick={()=>setConfirmDelete(c.complaint_id)} disabled={isActioning}
                variant="red" small={false}><Trash2 size={12}/> Delete</ActionBtn>
            )}
...
export default function NLPAnalysis() {
  ...
  const [fetchError,      setFetchError]     = useState(null)   // ← NEW: visible error
  const [apiOnline,       setApiOnline]      = useState(true)
  const [actionLoading,   setActionLoading]  = useState(null)
  const [confirmDelete,   setConfirmDelete]  = useState(null)
  const [selectedComp,    setSelectedComp]  = useState(null)
  const [lastRefreshed,   setLastRefreshed]  = useState(null)   // ← NEW: timestamp
  const [refreshTrigger,  setRefreshTrigger] = useState(0)      // ← NEW: drives auto-refresh
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // ← NEW: show the actual error instead of silently emptying the list
      const msg = err?.response?.data?.detail || err?.message || 'Unknown error'
      setFetchError(`API error: ${msg}`)
      setApiOnline(false)
      // Keep existing data — don't wipe the list on network hiccup
    } finally {
      setLoading(false)
    }
  }

  // Re-fetch when filters OR refresh trigger changes
  ...
  const handleStatusUpdate = async (complaintId, newStatus) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail || err?.message || 'check API'
    ...
    <div style={{ padding:'40px 48px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:48 }}>
        <span style={{ width:6, height:6, borderRadius:'50%', background:C.cyan,
          display:'inline-block', animation:'nlp-pulse 1.8s infinite' }}/>
        <span style={{ fontSize:9, fontWeight:800, letterSpacing:'2.5px',
          textTransform:'uppercase', color:C.cyan }}>{t('common.loading')}</span>
      </div>
      <Spinner size={48}/>
    </div>
  )
  ...
  const sentChart = stats?.by_sentiment&&Object.keys(stats.by_sentiment).length>0 ? {
    ...
    options: {
      ...
      plotOptions:{ pie:{ donut:{ size:'68%', labels:{ show:true,
        value:{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'26px', fontWeight:900, color:C.text },
        total:{ show:true, label:t('nlp.critiques'), fontSize:'10px', color:C.textMuted,
          formatter:()=>String(stats.by_sentiment?.critique||0) }
      }}}},
      legend:{ position:'bottom', fontSize:'11px', labels:{ colors:C.textMuted }, itemMargin:{ horizontal:8 }},
      dataLabels:{ enabled:false },
      tooltip:{ theme:'dark', y:{ formatter:v=>`${v} items` }},
    },
  } : null
  ...
  const catChart = stats?.by_category&&Object.keys(stats.by_category).length>0 ? {
    ...
    options:{
      ...
      colors: Object.keys(stats.by_category).map((_,i)=>{
        const p=[C.red,'#D41F35','#DA2E3C','#E04050','#E65060','#EC6070']; return p[i%p.length]
      }),
      xaxis:{ categories:Object.keys(stats.by_category), labels:{ style:{ fontSize:'9px', colors:C.textMuted }},
  ...
  const typeChart = (complaintCount>0||nonComplaintCount>0) ? {
    ...
    options:{
      ...
      plotOptions:{ pie:{ donut:{ size:'68%', labels:{ show:true,
        value:{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:'26px', fontWeight:900, color:C.text },
        total:{ show:true, label:'Rate', fontSize:'10px', color:C.textMuted,
    ...
    <div style={{ background:C.bg, minHeight:'100vh', color:C.text }}>
      ...
      <div style={{ padding:'40px 48px 80px', maxWidth:1600, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', alignItems:'flex-start', gap:12,
            ...
            <div>
              <div style={{ fontSize:12, color:C.redLight, fontWeight:700, marginBottom:4 }}>
                Fetch Error — données potentiellement incomplètes
              </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:1, background:'rgba(255,255,255,.04)' }}>
          {kpiTiles.map((kpi,i)=>(
            <StatBlock key={i} label={kpi.label} value={kpi.value}
              color={kpi.color} icon={kpi.Icon} sub={kpi.sub}/>
          ))}
        </div>
                ...
                {typeChart ? (
                  ...
                  <div style={{ height:240, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10 }}>
                    ...
                    <div style={{ fontSize:11, color:C.textDim, textAlign:'center', lineHeight:1.6 }}>
                      ...
                      <span style={{ fontSize:9, letterSpacing:'1px', textTransform:'uppercase', color:C.cyan }}>
                        {t('nlp.pythonRequired')}
                      </span>
                    </div>
                  </div>
                )}
        ...
        {/* Row 1 — Filters */}
        <div style={{ display:'flex', gap:1, background:'rgba(255,255,255,.04)',
          marginBottom:1, flexWrap:'wrap', alignItems:'stretch' }}>
          {FILTER_CONFIG.map(f=>(
            <div key={f.id} style={{ background:C.bg3, border:`1px solid ${C.border}`,
              padding:'10px 16px', display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ fontSize:9, fontWeight:800, color:C.textDim,
                letterSpacing:'2px', textTransform:'uppercase', whiteSpace:'nowrap' }}>{f.label}</span>
              <StyledSelect value={f.value} onChange={f.set} options={f.options} id={f.id}/>
            </div>
          ))}

          {/* ← NEW Refresh button with auto-refresh indicator */}
          <div style={{ background:C.bg3, border:`1px solid ${C.border}`,
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:1 }}>
              <span style={{ fontSize:8, color:C.textDim, letterSpacing:'1.5px', textTransform:'uppercase' }}>
                Dernière MAJ
              </span>
              <span style={{ fontSize:11, color:C.textMuted, fontFamily:"'Barlow Condensed',sans-serif",
          ...
          <div style={{ marginLeft:'auto', background:C.bg3, border:`1px solid ${C.border}`,
            ...
            <Link to="/form" target="_blank" rel="noreferrer"
              style={{ display:'inline-flex', alignItems:'center', gap:7, background:'transparent',
                color:C.textMuted, fontSize:11, fontWeight:600, textDecoration:'none',
        ...
        <div style={{ background:C.bg3, border:`1px solid ${C.border}`,
          ...
          <div style={{ position:'relative', flex:1, maxWidth:420 }}>
            <Search size={13} color={C.textDim} style={{ position:'absolute', left:10,
              top:'50%', transform:'translateY(-50%)', pointerEvents:'none' }}/>
            <input
              type="text"
              className="nlp-search"
          ...
          {searchQuery && (
            <button onClick={()=>setSearchQuery('')} style={{ background:'transparent',
              border:`1px solid ${C.border}`, color:C.textMuted, cursor:'pointer',
              padding:'6px 10px', fontSize:10, display:'flex', alignItems:'center', gap:4,
          ...
          <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6,
            fontSize:9, color:C.textDim, letterSpacing:'1px' }}>
            <span style={{ width:6, height:6, borderRadius:'50%', background:C.green,
              display:'inline-block', animation:'nlp-pulse 3s ease-in-out infinite' }}/>
            Auto-refresh 30s
          </div>
        </div>
              ...
              <thead>
                <tr style={{ background:'rgba(255,255,255,.025)', borderBottom:`1px solid ${C.border}` }}>
                  {[
                    ...
                    <th key={label} style={{ padding:'12px 12px', textAlign:'left', fontSize:9,
                      fontWeight:800, letterSpacing:'1.5px', textTransform:'uppercase',
                      color:C.textDim, whiteSpace:'nowrap' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                        {Icon && <Icon size={9} color={C.textDim}/>}
                        {label}
                      </div>
                    </th>
                  ))}
                ...
                {filteredComplaints.length === 0 ? (
                  <tr>
                    <td colSpan={11} style={{ padding:48, textAlign:'center', color:C.textMuted }}>
                      {searchQuery
                        ? <><Search size={20} color="rgba(255,255,255,.18)" style={{ display:'block', margin:'0 auto 12px' }}/>
                           Aucun résultat pour <strong style={{ color:C.text }}>"{searchQuery}"</strong>
                          </>
                    ...
                    <tr key={c.complaint_id||c.id}
                      ...
                      <td style={{ padding:'9px 12px' }}>
                        <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:13,
                          fontWeight:800, color:C.red, letterSpacing:'-.3px' }}>
                          {c.complaint_id}
                        </span>
                      </td>
                      ...
                      <td style={{ padding:'9px 12px', color:C.text, maxWidth:180,
                        overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}
                        title={c.text_original}>
                        {c.text_original}
                      </td>
                      <td style={{ padding:'9px 12px' }}>
                        <Badge variant={c.language==='ar'?'red':c.language==='fr'?'cyan':'green'}>
                          {c.language?.toUpperCase()}
                        </Badge>
                      </td>
                      <td style={{ padding:'9px 12px', color:C.textMuted, maxWidth:100,
                        overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontSize:10 }}>
                        {c.nlp_category}
                      </td>
                      <td style={{ padding:'9px 12px' }}>
                        <Badge variant={c.nlp_sentiment==='critique'?'red':c.nlp_sentiment==='négatif'?'amber':c.nlp_sentiment==='positif'?'green':'gray'}>
                          {c.nlp_sentiment}
                        </Badge>
                      </td>
                      <td style={{ padding:'9px 12px', whiteSpace:'nowrap' }}>
                        <span style={{ ...urgStyle, padding:'3px 8px', fontSize:9,
                      ...
                      <td style={{ padding:'9px 12px', color:C.textDim, fontSize:10 }}>
                        {c.nlp_city||'—'}
                      </td>
                      <td style={{ padding:'9px 12px', whiteSpace:'nowrap' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                          <span style={{ width:6, height:6, borderRadius:'50%',
                            background:STATUS_COLOR[c.status]||C.textDim, flexShrink:0 }}/>
                      ...
                      <td style={{ padding:'9px 12px' }} onClick={e=>e.stopPropagation()}>
                        <div style={{ display:'flex', gap:4, flexWrap:'nowrap' }}>
                          {c.status!=='in_progress'&&c.status!=='resolved'&&(
                            <ActionBtn onClick={()=>handleStatusUpdate(c.complaint_id,'in_progress')}
                              disabled={isActioning} variant="amber">{t('nlp.enCours')}</ActionBtn>
                          )}
                          {c.status!=='resolved'&&(
                            <ActionBtn onClick={()=>handleStatusUpdate(c.complaint_id,'resolved')}
                              disabled={isActioning} variant="green">
                          ...
                          {confirmDelete===c.complaint_id?(
                            <>
                              <ActionBtn onClick={()=>handleDelete(c.complaint_id)}
                                disabled={isActioning} variant="redSolid">{t('nlp.confirmer')}</ActionBtn>
                              <ActionBtn onClick={()=>setConfirmDelete(null)} variant="ghost">
                                <X size={9}/>
                              </ActionBtn>
                            </>
                          ):(
                            <ActionBtn onClick={()=>setConfirmDelete(c.complaint_id)}
                              disabled={isActioning} variant="red">
                              <Trash2 size={10}/>
                            </ActionBtn>
                          )}
                        </div>
