// src/api/client.js
// SpiriCom NOC — Centralized Axios client
...
const api = axios.create({
  ...
  timeout: 15_000,   // FIX-2: 15s for analytics (was 30s)
...
api.interceptors.response.use(
  ...
  err => {
    ...
    if (status === 401) {
      // FIX-3: clear storage but don't redirect — setupAxiosAuth handles it
...
export const analyticsApi = {
  ...
  anomaliesTimeline: (region) =>
    api.get('/api/analytics/anomalies/timeline', { params: { region } }),
...
// ════════════════════════════════════════════════════════════════════
// AI API — FIX-2: longer timeout for LLM calls
// ════════════════════════════════════════════════════════════════════
export const aiApi = {
  chat:      (payload) => api.post('/api/ai/chat',   payload,    { timeout: 120_000 }),
  status:    ()        => api.get('/api/ai/status'),
  getConfig: ()        => api.get('/api/ai/config'),
  saveConfig:(payload) => api.post('/api/ai/config', payload),
  resetUsage:()        => api.delete('/api/ai/config/reset-usage'),
}
