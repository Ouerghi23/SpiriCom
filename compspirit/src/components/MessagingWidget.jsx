...
// ─────────────────────────────────────────────────────────────────────
// SpiriCom NOC — Floating messaging widget (v3, aligned to UI.jsx)
//
...
//       in dark mode).
//  M-12 Message status ladder: Sent (✓ gray) → Delivered (✓✓ gray) →
//       Seen (✓✓ blue), replacing the old binary "Read" indicator.
//       Derived entirely from read_by.length — see F-3.
//
...
//  F-3  M-12's Sent/Delivered/Seen ladder assumes the backend seeds
//       read_by=[from_user] when a message is created (length 1 =
//       "delivered, not yet read"; length >1 = "seen"). If the backend
//       instead seeds read_by=[], messages will show "Sent" until the
//       recipient opens their panel — no crash, just one fewer visible
//       tier until confirmed against messages_api.py.
// ─────────────────────────────────────────────────────────────────────
...
// ── M-12: Sent / Delivered / Seen ladder — see F-3 for the assumption ──
const STATUS_META = {
...
export default function MessagingWidget() {
  ...
  // M-13: the API returns newest-first (ORDER BY id DESC, for pagination
  // efficiency). Reverse for display only, so the conversation reads
  // chronologically top-to-bottom like any chat app, with the newest
  // message at the bottom where bottomRef auto-scrolls to.
        ...
        <div className="msgw-panel" style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px',
            ...
            {loading ? (
              ...
              // M-12: Sent / Delivered / Seen ladder (own messages only)
              ...
              return (
                <div key={msg.id} className="msgw-item"
                  ...
                  <div style={{ maxWidth: '78%' }}>
                    ...
                    {/* M-12: status ladder — Sent (✓ gray) / Delivered
                        (✓✓ gray) / Seen (✓✓ blue). Own messages only. */}
                    {isMe && (
